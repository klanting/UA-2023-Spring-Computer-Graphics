...
namespace
{
	...
	void parse_rules(std::set<char> const& alphabet, std::map<char, std::vector<std::pair<double, std::string>>>& rules, stream_parser& parser, bool parse2D)
	{
		...
		while (true)
		{
			...
			rules[alphabet_char].push_back(make_pair(1, rule));
			parser.skip_comments_and_whitespace();
			c = parser.getChar();
			if (c == '}')
				break;
    ...
    void parse_rules_random(std::set<char> const& alphabet, std::map<char, std::vector<std::pair<double, std::string>>>& rules, stream_parser& parser, bool parse2D)
    {
        ...
        while (true)
        {
            ...
            if (d > 1){
                throw LParser::ParserException(std::string("Double is bigger than 1.0: ") + std::to_string(d) + "' in rules specification ", parser.getLine(), parser.getCol());
            }
            ...
            count[alphabet_char] += d;
            rules[alphabet_char].push_back(make_pair(d, rule));
            parser.skip_comments_and_whitespace();
            c = parser.getChar();
            if (c == '}')
                break;
        ...
        for (auto entry: count){
            if (entry.second-1 > 0.01 || entry.second-1 < -0.01){
                throw LParser::ParserException(std::string("total chance is bigger than 1 for '") + entry.first + "' in rule specification", parser.getLine(), parser.getCol());
            }
        }
    }
...
LParser::LSystem::LSystem(LSystem const&system) :
	alphabet(system.alphabet), drawfunction(system.drawfunction), initiator(system.initiator), angle(system.angle), replacementrules(system.replacementrules), nrIterations(system.nrIterations), do_stochastisch(system.do_stochastisch)
{
...
LParser::LSystem& LParser::LSystem::operator=(LParser::LSystem const& system)
{
    ...
    do_stochastisch = system.do_stochastisch;
	return *this;
}
...
std::string const& LParser::LSystem::get_replacement(char c) const
{
    ...
    if (do_stochastisch){
        ...
        std::uniform_int_distribution<int> change(0,1000);
        double drempel = change(device);
        double current = 0;
        int count = 0;
        for (auto r: replacementrules.find(c)->second){
            current += 1000.0*r.first;
            if (current >= drempel){
                return replacementrules.find(c)->second[count].second;
            }
            count += 1;

        }
        //safety return
        return replacementrules.find(c)->second[0].second;
    }else{
        return replacementrules.find(c)->second[0].second;
    }
...
void LParser::LSystem::SetRandom(bool b) {
    do_stochastisch = b;
}

bool LParser::LSystem::GetRandom() {
    return do_stochastisch;
}
...
std::istream& LParser::operator>>(std::istream& in, LParser::LSystem2D& system)
{
    ...
    if (system.GetRandom()){
        parse_rules_random(system.alphabet, system.replacementrules, parser, true);
    }else{
